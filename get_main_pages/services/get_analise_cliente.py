...
def get_dados_analise_cliente(data_inicial, data_final):
    try:
        ...
        atividades = get_atividades_empresa_mes(data_inicial, data_final)  # <== removido .json()

        # Indexação por código da empresa
        faturamento_dict = {str(item["codi_emp"]): item["faturamento"] for item in faturamentos}
        importacoes_dict = {str(item["codi_emp"]): item["importacoes"] for item in importacoes}
        contratados_dict = {str(item["codi_emp"]): item["quantidade_ativos"] for item in contratados}
